...
public class TicketMasterAPI implements ExternalAPI {
	...
	private static final String API_KEY = "yscamAv3bnofrzFXTqVGoOtMowBxeoyu";

	/**
	 * Creates and sends a request to the TicketMaster API by term and location.
	 */
	@Override
	public List<Item> search(double lat, double lon, String term, String radius) {
...
//			System.out.println("Print embeded: "+ embedded);
			JSONArray events = (JSONArray) embedded.get("events");
	//		JSONArray dates = (JSONArray) embedded.get("dates");
			System.out.println("Print events: "+ events);
	...
	private void queryAPI(double lat, double lon) {
		List<Item> itemList = search(lat, lon, null, "50");
		try {
...
//				System.out.println(venues);
				if (venues.length() >= 1) {
//					System.out.println(venues.getJSONObject(0));
	...
	private String getStartDate(JSONObject event) throws JSONException {
		if (!event.isNull("dates")) {
			...
			JSONObject startDate = dates.getJSONObject("start");
			return getStringFieldOrNull(startDate, "localDate");
			}
		}
		return null;
	}
	
	private String getEndDate(JSONObject event) throws JSONException {
		if (!event.isNull("dates")) {
			JSONObject dates = event.getJSONObject("dates");
			if (!dates.isNull("end")) {
			JSONObject startDate = dates.getJSONObject("end");
